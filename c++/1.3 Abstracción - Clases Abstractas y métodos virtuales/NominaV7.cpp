...
class Contribuyente {
  public:
   static constexpr float IMPUESTO_SOBRE_LA_RENTA = 0.16f;  
...
class VendedorBase : public Vendedor, public Contribuyente {
  private:
    string depto;
...
{ 
  ...
  for (int i=0; i<len; i++ ) {
    ...
    if(dynamic_cast<VendedorBase*>(vendedores[i])) {
      cout << vendedores[i]->getNombre() << ", Sueldo: $" << vendedores[i]->calculaSueldo() << ", ISR: $" << 
           (dynamic_cast<VendedorBase*>(vendedores[i]))->calculaISR(vendedores[i]->calculaSueldo()) << endl;
    }
...
// Compilar como: g++ -std=c++11 NominaV7.cpp -o NominaV7
